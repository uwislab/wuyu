...
<el-container>  <div class="app-container">
    ...
    <!-- 展示表格 -->
    <el-table :data="tableData" border stripestyle="width:100%">
    ...
    "form.gradeId": {
      handler(newval, oldval) {
        if (
          ...
          this.getclasslist(parseInt(newval - 1));
          // this.form.classId = "";
          console.log(1);
        } else if (newval === "") {
          this.classlist = [];
  ...
  methods: {
    ...
    handleSuccess(response) {
      if (response.code == 200) {
        ...
        this.$message.error("上传内容有误，请删除后尝试");
      }
    },
    beforeUpload(file) {
      console.log(file);
    ...
    httpRequest(e) {
      ...
      // else if( !this.beforeUpload(file)){
      //   return false
      // }
    ...
    async getclasslist(index) {
      ...
      let temp = this.gradeandclass[index];
      if (temp != null && index >= 0) {
        console.log(temp.grade.class);
    ...
    async getList(page = 1) {
      ...
      if (
        ...
        classId = this.searchObj.className.toString()[0];
      }
    ...
    async getClassID() {
      ...
      this.gradeandclass.shift();
      this.gradeandclass.push(temp);

      console.log(this.gradeandclass);
